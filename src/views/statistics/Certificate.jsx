...
export default function BarChart() {
  ...
  useEffect(() => {
    d3.csv(Data).then((csvData) => {
      ...
      let othersCount = 0;
      let othersTotalRating = 0;

      const majorData = dataArray.filter((d) => d.count >= MIN_COUNT_THRESHOLD);
      const minorData = dataArray.filter((d) => d.count < MIN_COUNT_THRESHOLD);

      minorData.forEach((d) => {
        othersCount += d.count;
        othersTotalRating += d.totalRating;
      });
      ...
      if (othersCount > 0) {
        const othersData = {
          certificate: "Others",
          count: othersCount,
          totalRating: othersTotalRating,
          avgRating: othersTotalRating / othersCount,
        };
        majorData.push(othersData);
      }

      dataArray = majorData.sort((a, b) => b.avgRating - a.avgRating);
